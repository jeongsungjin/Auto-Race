#include "obstacle_detector/obstacle_extractor_parking.h"
...
ObstacleExtractor::ObstacleExtractor(ros::NodeHandle& nh, ros::NodeHandle& nh_local) : nh_(nh), nh_local_(nh_local) {
  ...
  roi_points_pub_ = nh_.advertise<sensor_msgs::PointCloud>("roi_points_parking", 10);
...
bool ObstacleExtractor::updateParams(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res) {
  ...
  if (p_active_ != prev_active) {
    if (p_active_) {
      ...
      obstacles_pub_ = nh_.advertise<obstacle_detector::Obstacles>("raw_obstacles_parking", 10);
    }
    else {
      // Send empty message
